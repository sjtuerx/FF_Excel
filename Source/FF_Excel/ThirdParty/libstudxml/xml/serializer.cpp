 ...
 /* void serialization::
      ...
      : os_ (os), os_state_ (os.exceptions ()), oname_ (oname), depth_ (0)
  {
    ...
    if (genxStatus e = genxStartDocSender (s_, &sender_))
    {
      ...
      genxDispose (s_);
	  assert(false);//throw serialization (oname, m);
    }
  }
  ...
  handle_error (genxStatus e) const
  {
    ...
    case GENX_IO_ERROR:
      ...
      // Fall through.
    default:
		assert(false);//throw serialization (oname_, genxGetErrorMessage (s_, e));
    }
  }
  ...
  {
    constUtf8 cns, cn;
	genxStatus e = genxGetCurrentElement(s_, &cns, &cn);
    if ((e ) ||
  ...
  end_attribute (const string& ns, const string& name)
  {
    constUtf8 cns, cn;
    genxStatus e;
	e = genxGetCurrentAttribute(s_, &cns, &cn);
    if ((e) ||
        reinterpret_cast<const char*>(cn) != name ||
        (cns == 0 ? !ns.empty () : reinterpret_cast<const char*> (cns) != ns))
    {
