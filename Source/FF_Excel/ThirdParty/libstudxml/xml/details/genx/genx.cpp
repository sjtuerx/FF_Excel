...
static utf8_string copy(genxWriter w, constUtf8 from)
{
  utf8_string temp;

  if ((temp = (utf8_string) allocate(w, strlen((const char *) from) + 1)) == NULL)
    return NULL;
...
{
  c->space = 100;
  if ((c->buf = (utf8_string) allocate(w, c->space)) == NULL)
...
{
  utf8_string newSpace;

  c->space = size * 2;
  if ((newSpace = (utf8_string) allocate(w, c->space)) == NULL)
    return GENX_ALLOC_FAILED;
...
static genxElement findElement(plist * pl, constUtf8 xmlns, constUtf8 name)
{
  ...
  for (i = 0; i < pl->count; i++)
  {
    ...
    {
      if (ee[i]->ns != NULL &&
          (xmlns== ee[i]->ns->name||strcmp((const char *) xmlns, (const char *) ee[i]->ns->name) == 0) &&
 ...
 *  is set.
 */
static utf8_string storePrefix(genxWriter w, constUtf8 prefix, Boolean force)
{
  int high, low;
  utf8_string * pp = (utf8_string *) w->prefixes.pointers;
  unsigned char buf[1024];

  if (prefix[0] == 0)
    prefix = (utf8_string) "xmlns";
  else
  {
...
genxWriter genxNew(genxAlloc alloc, genxDealloc dealloc, void * userData)
{
  ...
  if ((w->empty = copy(w, (utf8_string) "")) == NULL)
  {
    w->status = GENX_ALLOC_FAILED;
    return NULL;
  }

  w->xmlnsEquals = declareAttribute(w, NULL, (utf8_string) "xmlns", NULL, &w->status);
  ...
  xml = genxDeclareNamespace(w, (utf8_string) "http://www.w3.org/XML/1998/namespace",
			     (utf8_string) "xml", &w->status);
  if (xml == NULL)
    return NULL;
  xml->declCount = 1;
 ...
 * Purify some text
 */
int genxScrubText(genxWriter w, constUtf8 in, utf8_string out)
{
  int problems = 0;
  constUtf8 last = in;
				   ...
				   genxStatus * statusP)
{
  ...
  /* previously declared? */
  ns = findNamespace(&w->namespaces, uri);
  if (ns)
  {
 ...
 */
utf8_string genxGetNamespacePrefix(genxNamespace ns)
{
                                         ...
                                         constUtf8 start, constUtf8 end)
{
  ...
  utf8_string last = (utf8_string) start;
...
genxStatus genxAddText(genxWriter w, constUtf8 start)
{
  ...
  if (w->sequence == SEQUENCE_CONTENT)
  {
    ...
    return sendxBounded(w, breaker, (utf8_string) start);
  }
...
genxStatus genxAddBoundedText(genxWriter w, constUtf8 start, constUtf8 end)
{
  ...
  if (w->sequence == SEQUENCE_CONTENT)
  {
    while (start < end)
    {
      ...
      w->status = addChar(w, c, (utf8_string) start, &lasts, &breaker);
      if (w->status != GENX_SUCCESS)
        return w->status;
    }
    return sendxBounded(w, breaker, (utf8_string) start);
...
{
  utf8_string end = (utf8_string) (start + byteCount);
...
genxStatus genxAddCharacter(genxWriter w, int c)
{
  ...
  utf8_string lasts, breaker, next;
  ...
  {
    *next++ = (unsigned char)(0xc0 | (c >> 6));
    *next++ = (unsigned char)(0x80 | (c & 0x3f));
  }
  else if (c < 0x10000)
  {
    *next++ =(unsigned char)( 0xe0 | (c >> 12));
    *next++ =(unsigned char)( 0x80 | ((c & 0xfc0) >> 6));
    *next++ =(unsigned char)( 0x80 | (c & 0x3f));
  }
  else
  {
    *next++ = (unsigned char)(0xf0 | (c >> 18));
    *next++ = (unsigned char)(0x80 | ((c & 0x3f000) >> 12));
    *next++ = (unsigned char)(0x80 | ((c & 0xfc0) >> 6));
    *next++ = (unsigned char)(0x80 | (c & 0x3f));
  }
  *next = 0;
                                  ...
                                  constUtf8 is)
{
  ...
  if (pi2 != NULL && (w->status = genxCheckText(w, pi2)) != GENX_SUCCESS)
    return w->status;
...
genxStatus genxComment(genxWriter w, constUtf8 text)
{
      ...
      w->sequence == SEQUENCE_ATTRIBUTES)
  {
    ...
    if ((w->status = sendx(w, (utf8_string) "\n")) != GENX_SUCCESS)
      return w->status;

  if ((w->status = sendx(w, (utf8_string) "<!--")) != GENX_SUCCESS)
    return w->status;
  if ((w->status = sendx(w, (utf8_string) text)) != GENX_SUCCESS)
    return w->status;
  if ((w->status = sendx(w, (utf8_string) "-->")) != GENX_SUCCESS)
    return w->status;
