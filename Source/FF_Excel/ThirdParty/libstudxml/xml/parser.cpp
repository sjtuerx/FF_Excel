...
namespace xml
{
 ...
 /* void parsing::init()
  ...
  {
    if (next () != e)
		assert(false);// throw parsing (*this, string (parser_event_str[e]) + " expected");
  }
  ...
  next_expect (event_type e, const string& ns, const string& n)
  {
    if (next () != e || namespace_ () != ns || name () != n)
		assert(false);//throw parsing (*this,string (parser_event_str[e]) + " '" +qname_type (ns, n).string () + "' expected");
  }
  ...
  pop_element ()
  {
    ...
    if (e.attr_unhandled_ != 0)
    {
      ...
      {
        if (!i->second.handled)
			assert(false);//throw parsing (*this, "unexpected attribute '" + i->first.string () + "'");
      }
      assert (false);
    }
  ...
  next_ (bool peek)
  {
    ...
    case start_element:
      {
        if (const element_entry* e1 = get_element ())
        {
          switch (e1->content)
          {
          case content_type::empty:
			  assert(false);//throw parsing (*this, "element in empty content");
          case content_type::simple:
			  assert(false);//throw parsing (*this, "element in simple content");
          default:
            break;
          }
        }
    ...
    do
    {
      ...
      else
      {
        ...
        if (is.bad () || (is.fail () && !is.eof ()))
			assert(false);//throw parsing (*this, "io failure");
      ...
      if (am || ae)
      {
        for (; *atts != 0; atts += 2)
        {
          if (am)
          {
            qname_type qn;
            split_name (*atts, qn);
            attribute_map_type::value_type v2 (qn, attribute_value_type ());
			v2.second.value = *(atts + 1);
			v2.second.handled = false;
            pe->attr_map_.insert (v2);
          }
          else
          {
